...
<form name="form_new_product" id="form_new_product" action="" method="POST" class="validation widget">
	<div class="header">Hesap Kartı</div>
    <div class="content">
        <div class="row">
            <div class="col-md-6">
        ...
        <?php if($account['status'] == 1): ?>
            ...
            <?php if(get_the_current_user('role') <= $options['who_can_edit_account_card']['option_value'] or $options['who_can_edit_account_card']['option_value'] == ''): ?>
            <div class="h20"></div>
            <div class="text-right">
                <input type="hidden" name="log_time" value="<?php echo logTime(); ?>" />
                <input type="hidden" name="update" />
                <button class="btn btn-default btn-lg"><?php lang('Update'); ?> &raquo;</button>
            </div> <!-- /.text-right -->
            <?php endif; ?>
        <?php endif; ?>
	</div> <!-- /.content -->
...
<table class="table table-bordered table-hover table-condensed dataTable" exportname="cari_ekstre">
    ...
    <?php foreach($invoices as $invoice): ?>
    	<tr>
            ...
            <td><?php echo get_text_invoice_type($invoice['type']); ?></td>
...
<table class="table table-bordered table-hover table-condensed dataTable2" exportName="cari_detayli_ekstre">
	...
	$this->db->order_by('date', 'ASC');
	$this->db->where('account_id', $account['id']);
	$invoice_items = $this->db->get('fiche_items')->result_array();
	$balance = 0;
	?>
    <?php foreach($invoice_items as $item): ?>
    	<tr>
        	<td><a href="<?php echo site_url('invoice/view/'.$item['invoice_id']); ?>" target="_blank"><?php echo $item['invoice_id']; ?></a></td>
        	<td class="hide"></td>
            <td class="fs-11"><?php echo substr($item['date'],0,10); ?></td>
            <td class="fs-10"><?php echo get_text_invoice_type($item['type']); ?></td>
            ...
            <td class="text-center fs-11"><?php echo the_amount($item['quantity']); ?></td>
    ...
    <tfoot>
		...
		$grand_total = $this->db->get('fiches')->row_array();
		?>
        <tr>
        	<th colspan="5"></th>
