...
{
	$GLOBALS['products'][$product['id']] = $product;	
}
...
function the_amount($quantity)
{
	echo get_amount($quantity);
}
...
function calc_product($product_id)
{
	...
	$sales = $ci->db->get('fiche_items')->row_array();
	
	$ci->db->where('product_id', $product_id);
	$ci->db->where('status', 1);
	$ci->db->where('in_out', '0');
	$ci->db->select_sum('quantity');
	$buying = $ci->db->get('fiche_items')->row_array();
